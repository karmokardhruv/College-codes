% MATLAB code for visualizing local maxima and minima (using first and second derivative test )and visualizing concavity this code will work for polynomial of degree 2 or more.
...
for i = 1:1:size(c)
    ...
    T3 = subs(f,x, c(i));
    if(double(T1)==0)
        fprintf('The point x is %d inflection point\n',double(c(i)));
    else
        if(double(T1)<0)
